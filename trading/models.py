...
class RetailNetwork(BaseModel):
    ...
    name = models.CharField(max_length=100)
    contacts = models.OneToOneField(Contacts, on_delete=models.CASCADE, related_name='retail_network')
    products = models.ManyToManyField(Products)
    provider_factory = models.ForeignKey(
        Factory,
        on_delete=models.SET_NULL,
        **NULLABLE,
        related_name='supplied_retail_networks'
    )
    debt = models.DecimalField(
        max_digits=15,
        decimal_places=2,
...
class Trader(BaseModel):
    ...
    provider_retail_network = models.ForeignKey(
        RetailNetwork,
        on_delete=models.SET_NULL,
        **NULLABLE,
        related_name='provider_for_traders'
    ...
    def get_provider(self):
        ...
        if self.provider_factory:
            return self.provider_factory.name
        elif self.provider_retail_network:
            return self.provider_retail_network.name
        return 'Нет поставщика'
